...
@rule
async def get_interpreter(pyproject: PythonProject) -> PythonInterpreter:
    interpreter_version = pyproject.interpreter_version
    # Install the interpreter into the named cache (or verify that it exists).
    uv_result = await execute_uv_process(
        UvProcess(
            "Install interpreter",
            ("python", "install", "--no-config", "--no-registry", "--no-bin", interpreter_version),
            None,
        )
    )
    ...
    uv_result = await execute_uv_process(
        UvProcess(
            ...
            ("python", "find", "--managed-python", interpreter_version),
            None,
            in_shell=True,
            post_command="echo ${PWD}",
        )
    )
    full_interpreter_path, _, sandbox_dir = (
        uv_result.process_result.stdout.decode().strip().partition("\n")
    )
    rel_interpreter_path = os.path.relpath(
        os.path.realpath(full_interpreter_path), os.path.realpath(sandbox_dir)
    )
