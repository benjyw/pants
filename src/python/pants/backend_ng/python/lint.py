...
async def lint_partition(
    source_paths: SourcePaths, lint_subsystem: LintSubsystem, ruff: Ruff
) -> CodeQualityResult:
    sources_digest = (await source_paths_to_digest(source_paths)).digest
    ...
    uv_args = [
        "tool",
        "run",
        *(f"--with={req}" for req in ruff_reqs.requirement_strings),
        "ruff",
        "check",
    ...
    if uv_result.exit_code not in {0, 1}:
        raise Exception(
            f"`uv tool run ruff` exited with code {uv_result.exit_code} due to: {uv_result.stderr}"
        )
...
async def do_lint(specs: Specs) -> CodeQualityResults:
    ...
    partition_results = await concurrently(
        lint_partition(**implicitly({partition: SourcePartition})) for partition in partitions
    )
