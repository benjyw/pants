...
@rule
async def execute_uv_process(
    uv_process: UvProcess, system: System, python: PythonProject, uv: Uv
) -> UvProcessResult:
    ...
    if uv_process.in_shell:
        ...
        sh_script = (
            args_str
            if uv_process.post_command is None
...
async def execute_uv_command(uv_command: UvCommand) -> UvProcessResult:
    ...
    descr_suffix = (
        f"{source_path_strs[0]}"
        if len(source_path_strs) == 1
        else f"{len(source_path_strs)} files under {common_dir}"
    )
